...
RSpec.describe Track::Month, type: :model do
  ...
  describe '#chart' do
    ...
    it 'processes previous positions' do
      ...
      Track::Month.chart
      expect(@track1.reload.previous_positions).to eq('Month' => { 'position' => 1 })
      expect(@track2.reload.previous_positions).to eq('Month' => { 'position' => 2 })
    end
    ...
    it 'processes position changes for email' do
      ...
      expect(Track::Charted.process_position_changes(@track1)).to eq('Month' => { 'position' => 1 }, 'top_ten' => true)
      expect(Track::Charted.process_position_changes(@track2)).to eq('Month' => { 'position' => 2 }, 'top_ten' => true)
    end
  end
end
